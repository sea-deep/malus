...
//! - Directly maps Apple entities (song, album, artist, playlist, station) to `AppleEntityRefWire`.
//! - Attaches typed `AppleActionWire` actions (PlaySong, PlayAlbum, PlayPlaylist, PlayStation).
...
use malus_protocol::wire::{
    AppleActionWire, AppleEntityRefWire, PageBadgeWire, PageItemWire, PageSectionWire,
};
...
pub fn map_apple_resource_to_item(item: &Value) -> Option<PageItemWire> {
    ...
    match typ {
        "songs" | "library-songs" => {
            ...
            page_item.entity = Some(AppleEntityRefWire::Song(id.to_string()));
            page_item.actions = vec![AppleActionWire::PlaySong(id.to_string())];
        ...
        "albums" | "library-albums" => {
            ...
            page_item.artwork = attrs.get("artwork").and_then(parse_apple_artwork);
            page_item.entity = Some(AppleEntityRefWire::Album(id.to_string()));
            page_item.open_route = Some(format!("apple:page:album:{id}"));
            page_item.actions = vec![AppleActionWire::PlayAlbum(id.to_string())];
        ...
        "artists" | "library-artists" => {
            ...
            page_item.artwork = attrs.get("artwork").and_then(parse_apple_artwork);
            page_item.entity = Some(AppleEntityRefWire::Artist(id.to_string()));
            page_item.open_route = Some(format!("apple:page:artist:{id}"));
        ...
        "playlists" | "library-playlists" => {
            ...
            page_item.subtitle = attrs
                ...
                .map(str::to_string);
            page_item.artwork = attrs.get("artwork").and_then(parse_apple_artwork);
            page_item.entity = Some(AppleEntityRefWire::Playlist(id.to_string()));
            page_item.open_route = Some(format!("apple:page:playlist:{id}"));
            page_item.actions = vec![AppleActionWire::PlayPlaylist(id.to_string())];
            page_item.presentation_hint = Some("card".to_string());
        ...
        "stations" => {
            ...
            page_item.subtitle = attrs
                ...
                .map(str::to_string);
            page_item.artwork = attrs.get("artwork").and_then(parse_apple_artwork);
            page_item.entity = Some(AppleEntityRefWire::Station(id.to_string()));
            page_item.actions = vec![AppleActionWire::PlayStation(id.to_string())];
            page_item.presentation_hint = Some("card".to_string());
...
mod tests {
    ...
    fn test_map_song_resource() {
        ...
        assert_eq!(
            item.entity,
            Some(AppleEntityRefWire::Song("1440857781".to_string()))
        );
        assert_eq!(
            item.actions,
            vec![AppleActionWire::PlaySong("1440857781".to_string())]
        );
    ...
    fn test_map_album_resource() {
        ...
        assert_eq!(
            item.entity,
            Some(AppleEntityRefWire::Album("1440857780".to_string()))
        );
        assert_eq!(
            item.open_route.as_deref(),
            Some("apple:page:album:1440857780")
        );
        assert_eq!(
            item.actions,
            vec![AppleActionWire::PlayAlbum("1440857780".to_string())]
        );
    }
}
