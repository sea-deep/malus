...
//!
//! Provides in-process Apple Music operations directly to `malus-daemon`:
...
use malus_protocol::{
    ...
    wire::{
        ...
        LibraryPageWire, PageContinuationWire, PageCursorWire, PageWire, SearchKindWire,
...
use crate::{
    ...
    auth::AuthState,
    error::AppleError,
    surfaces::{self, manifest::apple_navigation},
...
impl AppleService {
    ...
    /// Legacy capability list for test parity.
        ...
        match self.session.probe_auth().await {
            Ok(state) => {
                *self.current_auth_state.lock().await = state;
                Ok(state.to_status("apple", None))
            }
            Err(AppleError::ProfileBusy) => Ok(AuthState::Checking
                .to_status("apple", Some("Profile currently in use".to_string()))),
            Err(e) => Err(e),
        }
    }
    ...
    pub async fn auth_logout(&self) -> Result<AuthStatusWire, AppleError> {
        ...
        *self.current_auth_state.lock().await = AuthState::NeedsAuth;
        Ok(AuthState::NeedsAuth.to_status("apple", Some("Logged out successfully".to_string())))
    }
    ...
    /// Play a track by MediaId (e.g. `apple:track:617154362` or raw catalog ID).
    pub async fn play(&self, media_id: &str) -> Result<(), AppleError> {
        info!("Apple Music play: {media_id}");
        let catalog_id = if let Ok(mid) = malus_protocol::MediaIdWire::parse(media_id) {
            if mid.provider() != "apple" {
    ...
    pub fn get_navigation(&self) -> AppleNavigationWire {
        apple_navigation()
    }

    /// Return a structured Apple page (Home, New, Radio, Library, Album/Artist/Playlist detail, Replay).
    pub async fn get_page(&self, route_or_id: &str) -> Result<ApplePageWire, AppleError> {
        surfaces::get_apple_page(&self.api, route_or_id).await
    }
    ...
    pub async fn continue_page(
        ...
        surfaces::continue_apple_page(&self.api, route_or_id, cursor).await
    }
}

// Backwards compatibility alias
pub type AppleProvider = AppleService;
