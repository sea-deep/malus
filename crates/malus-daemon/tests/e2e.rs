//! End-to-end integration tests between malus-daemon and malus-client.
//!
//! Enforces:
...
use malus_daemon::{Engine, Server};
use malus_protocol::{
    PlaybackStateWire,
...
async fn test_daemon_navigation_and_pages_over_ipc() {
    ...
    // 2. GetProviderSurfaceManifest backwards-compatible alias
    let resp = client
        ...
        .await
        .unwrap();
    if let ClientResponse::ProviderSurfaceManifest(nav) = resp {
        assert_eq!(nav.default_route, "apple:page:home");
    } else {
        panic!("Expected ProviderSurfaceManifest response, got {resp:?}");
    }
}
...
async fn test_daemon_playback_control_lifecycle() {
    ...
    let resp = client
        .send(&ClientRequest::PlayTrack {
            media_id: "apple:track:617154362".to_string(),
        })
        .await
        .unwrap();
