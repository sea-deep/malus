...
/// Provider-neutral decoded image buffer. Zero GDK/GTK dependencies.
#[derive(Debug, Clone)]
...
/// Application-scoped artwork service.
#[derive(Clone)]
...
impl ArtworkService {
    ...
    /// Load decoded image by URL.
    /// Deduplicates in-flight requests, checks LRU and disk cache, limits concurrency.
    pub async fn load(&self, url: &str) -> Option<Arc<DecodedImage>> {
        ...
        if let Some(ref mut rx) = rx {
            // Another worker is already fetching this URL — await its broadcast result
            return rx.recv().await.unwrap_or_default();
        }

        // 3. We are the leader for this URL. Fetch and decode with concurrency permit.
        let result = self.fetch_and_decode(url).await;

        // 4. Update LRU if successful
        if let Some(ref img) = result {
            let mut lru = self.lru.lock().await;
            lru.insert(url.to_string(), img.clone());
        }

        // 5. Broadcast result to all waiting requesters and remove from in-flight
        {
            let mut in_flight = self.in_flight.lock().await;
    ...
    async fn fetch_and_decode(&self, url: &str) -> Option<Arc<DecodedImage>> {
        ...
        if let Some(ref path) = disk_path
            ...
            && !bytes.is_empty()
            && let Some(decoded) = decode_bytes_blocking(bytes).await
        {
        ...
        // Enforce content length limit if declared
        if let Some(len) = resp.content_length()
            && len as usize > MAX_IMAGE_RESPONSE_BYTES
        {
            return None;
        }

        let bytes = resp.bytes().await.ok()?.to_vec();
        if bytes.len() > MAX_IMAGE_RESPONSE_BYTES || bytes.is_empty() {
            return None;
        }
        ...
        let decoded = decode_bytes_blocking(bytes).await?;
        Some(Arc::new(decoded))
    }

    fn disk_cache_path(&self, url: &str) -> Option<PathBuf> {
        self.cache_dir.as_ref().map(|dir| {
            let hash = fnv1a_hash(url.as_bytes());
            dir.join(format!("{hash:016x}.bin"))
        })
    }
}

async fn decode_bytes_blocking(bytes: Vec<u8>) -> Option<DecodedImage> {
