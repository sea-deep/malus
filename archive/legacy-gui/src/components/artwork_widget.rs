//! Reusable ArtworkWidget Component.
//!
//! Owns visual state, requested URL, and in-flight generation.
//! Delegates fetch and decode to the shared `ArtworkService`.
//! Instantiates `gdk::MemoryTexture` on the GTK main thread.

use relm4::gtk::{self, prelude::*};
use relm4::prelude::*;
use std::sync::Arc;

use crate::artwork::{ArtworkService, DecodedImage};
    ...
    fn update_cmd(
        ...
        match message {
            ArtworkCmd::Loaded { generation, image } => {
                // Drop stale responses if URL changed while in flight
