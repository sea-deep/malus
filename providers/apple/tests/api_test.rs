...
async fn test_request_headers_and_path_construction() {
    ...
    assert_eq!(tracks.items[0].id, "apple:track:12345");
...
async fn test_auth_retry_on_401_success() {
    ...
    let item = api
        .get_catalog_item("apple:album:1440857780")
        .await
        .expect("should succeed after 401 refresh");

    match item {
        CatalogItemWire::Album(alb) => {
            assert_eq!(alb.id, "apple:album:1440857780");
...
async fn test_persistent_401_returns_auth_required() {
    ...
    let err = api.get_catalog_item("apple:track:123").await.unwrap_err();
...
async fn test_error_status_mappings() {
    // 403 Forbidden
    {
        ...
        let err = api.get_catalog_item("apple:track:1").await.unwrap_err();
    ...
    // 404 Not Found
    {
        ...
        let err = api
            .get_catalog_item("apple:track:nonexistent")
            .await
            .unwrap_err();
...
async fn test_collection_and_library_calls() {
    // 1. Collection items (catalog album tracks)
    {
        ...
        let page = api
            .get_collection_items("apple:album:1440857780", 2, None)
            .await
            .expect("collection items");

        assert_eq!(page.items.len(), 2);
        assert_eq!(page.items[0].id, "apple:track:1001");
        assert_eq!(page.items[0].title, "Give Life Back to Music");
        assert_eq!(page.items[1].id, "apple:track:1002");
        assert_eq!(
    ...
    // 2. Library songs
    {
        ...
        match page {
            LibraryPageWire::Tracks(p) => {
                ...
                assert_eq!(p.items[0].id, "apple:track:i.123456");
...
fn test_canonical_parsers() {
    ...
    assert_eq!(playlist.id, "apple:playlist:pl.u-1234");
