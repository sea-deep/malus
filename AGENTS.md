...
2. Mock content is only for tests. No fabricated stations, lyrics, listening history, audio-quality badges, or subscription claims.
...
12. Design Tokens and Theming Integrity: Never hardcode colors, dimensions, spacings, or radii in UI components. Colors must derive from the GTK/Libadwaita semantic theme palette (`@accent_color`, `@window_bg_color`, `@window_fg_color`, `@slider_bg_color`, etc.) or `FruitPalette`. Dimensions, paddings, icon glyph sizes, and layout rhythms must be referenced from `malus_gtk::design::tokens` (governed strictly by the 4px base grid and 8px rhythm).
